...
SELECT id FROM orders WHERE customer_id IN(SELECT id FROM customers WHERE id = 1);
...
SELECT T2.name, SUM(T1.num_cupcakes) "sum" FROM orders T1 RIGHT JOIN cupcakes T2 ON T1.cupcake_id = T2.id GROUP BY T2.name order by 1;
...
SELECT customers.email FROM customers LEFT JOIN orders ON customers.id = orders.customer_id GROUP BY customers.email HAVING SUM(orders.num_cupcakes) > 100 ORDER BY customers.email desc;
...
SELECT DISTINCT customers.fname, customers.lname, customers.email FROM customers INNER JOIN orders ON customers.id = orders.customer_id AND orders.processed=True AND orders.cupcake_id = 5;